...
public class CargarDatos {
    ...
    private void cargarPausas() {
        try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream("ms-viaje/src/main/resources/BDData/pausas.csv");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (datos.length >= 4) {
                    ...
                    pausa.setFechaHoraInicio(pausa.parsearFecha(datos[2]));
                    pausa.setFechaHoraFin(pausa.parsearFecha(datos[3]));
    ...
    private void cargarViajes() throws IOException {
        ...
        try (FileReader reader = new FileReader(viajesCSV);
            ...
            for (CSVRecord record : csvParser) {
                ...
                viaje.setFechaHoraInicio(Viaje.parsearFecha(record.get("fecha_hora_inicio")));

                if (record.isMapped("fecha_hora_fin") && !record.get("fecha_hora_fin").isEmpty()) {
                    viaje.setFechaHoraFin(Viaje.parsearFecha(record.get("fecha_hora_fin")));
                }
